...
export default function MyOrdersPage() {
  ...
  // [NÂNG CẤP] Trì hoãn việc tìm kiếm để mượt hơn
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedSearchTerm(searchTerm);
    }, 500); // Chỉ tìm sau khi người dùng ngừng gõ 500ms
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const normalizedStatus =
      order.status === "đang xử lý" ? "pending" : order.status;
    const matchStatus =
      activeTab === "all"
        ? true
        : order.payment_status === activeTab || normalizedStatus === activeTab;
  ...
  const renderStatusBadge = (rawStatus: string) => {
    const status = rawStatus === "đang xử lý" ? "pending" : rawStatus;
    switch (status) {
      case "pending":
        return (
          <span className="text-orange-500 font-bold flex items-center gap-1">
            <ShoppingBag size={14} /> Đang xử lý
          </span>
        );
      case "paid":
        return (
      ...
      case "returned":
        return (
          <span className="text-gray-500 font-bold flex items-center gap-1">
            <RefreshCcw size={14} /> Trả hàng
          </span>
        );
      default:
        return <span className="text-gray-500">Chờ cập nhật</span>;
    }
  };
              ...
              >
                ...
                <div className="flex gap-4 mb-4">
                  <div className="w-16 h-16 bg-gray-100 rounded-lg flex-shrink-0 overflow-hidden border">
                    ...
                    {order.items &&
                    (order.items[0]?.img || order.items[0]?.image) ? (
                      <img
                        src={order.items[0]?.img || order.items[0]?.image}
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-sm text-gray-800 line-clamp-1">
                      {/* [ĐÃ SỬA] Đọc tên từ items */}
                      {order.items && order.items.length > 0
                        ? order.items[0].title || order.items[0].name
                        : "Sản phẩm chưa đặt tên"}
                    </h4>
                    <p className="text-xs text-gray-500 mt-1">
                      {/* [ĐÃ SỬA] Đọc số lượng từ items */}
                      {order.items && order.items.length > 1
                        ? `và ${order.items.length - 1} sản phẩm khác`
                        : `Số lượng: ${order.items && order.items.length > 0 ? order.items[0].quantity : 1}`}
                    </p>
                  </div>
                  <div className="text-right">
                    {/* [ĐÃ SỬA] Lấy total_amount của đơn hàng */}
