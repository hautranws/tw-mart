...
export default function ProductManagementPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const { count } = await supabase
        .from("products_tw") // Sửa: Lấy từ kho hàng Đài Loan
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      // 1. Lấy thông tin sản phẩm để có link ảnh
      ...
      if (fetchError)
        throw new Error(
          "Không tìm thấy sản phẩm để xóa. " + fetchError.message,
        );
      ...
      if (deleteError)
        throw new Error(
          "Lỗi xóa sản phẩm khỏi database: " + deleteError.message,
        );

      // 3. Xóa ảnh trong Storage (nếu có)
      if (productToDelete?.img) {
        let imageUrls: string[] = [];
        try {
          imageUrls = JSON.parse(productToDelete.img);
        } catch {
          imageUrls = [productToDelete.img];
        }
        ...
        if (Array.isArray(imageUrls) && imageUrls.length > 0) {
          const filePaths = imageUrls
            .map((url) => {
              try {
                ...
                const bucketIndex = pathSegments.indexOf("tw-mart-products");
                if (bucketIndex > -1) {
          ...
          if (filePaths.length > 0) {
            await supabase.storage.from("tw-mart-products").remove(filePaths);
          }
        }
      }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-blue-900">
            📦 QUẢN LÝ KHO HÀNG ĐÀI LOAN (Trang {currentPage})
          </h1>
          <Link
            href="/admin/add"
